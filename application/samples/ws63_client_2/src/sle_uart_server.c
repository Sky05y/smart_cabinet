...
// ===== 支持多连接的发送函数 =====
errcode_t sle_uart_server_send_report_by_conn_id(uint16_t conn_id, const uint8_t *data, uint16_t len)
{
    ssaps_ntf_ind_t param = {0};
    ...
    osal_printk("[SLE] ssaps_notify_indicate: server_id=%d, conn_id=0x%02x, len=%d\r\n", 
        g_server_id, conn_id, len);
