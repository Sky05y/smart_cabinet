...
void trigger_unlock(void)
{
    osal_printk("[LOCK] Step 1: Relay ON\r\n");
    uapi_gpio_set_val(RELAY_CTRL_PIN, RELAY_ON_LEVEL);
    
    osDelay(1000);
    osal_printk("[LOCK] Step 2: 1s passed\r\n");
    
    osDelay(1000);
    osal_printk("[LOCK] Step 3: 2s passed\r\n");
    
    osDelay(1000);
    osal_printk("[LOCK] Step 4: 3s passed\r\n");
    
    uapi_gpio_set_val(RELAY_CTRL_PIN, RELAY_OFF_LEVEL);
    osal_printk("[LOCK] Step 5: Relay OFF\r\n");
}